...
from .serializers import (
    ...
    CartSerializer, CartDetailSerializer, CartCreateSerializer, 
    CartItemSerializer, CartItemCreateSerializer, CartConvertToOrderSerializer
)
...
class CartViewSet(viewsets.ModelViewSet):
    """
    API para gestión de carritos de compras temporales.
    Maneja el estado del carrito antes de convertirlo en órdenes.
    """
    queryset = Cart.objects.select_related('table__zone').prefetch_related(
        'cartitem_set__recipe__group',
        'cartitem_set__container'
    ).order_by('-updated_at')
    permission_classes = []  # Acceso completo para todos los usuarios autenticados
    pagination_class = None  # Deshabilitar paginación
    ...
    def create(self, request, *args, **kwargs):
        """Override create to return detailed cart after creation"""
        serializer = self.get_serializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        cart = serializer.save()
        
        # Return cart with full details
        response_serializer = CartDetailSerializer(cart)
        return Response(response_serializer.data, status=status.HTTP_201_CREATED)
        ...
        if serializer.is_valid():
            try:
                ...
                if existing_item:
                    ...
                    existing_item.save()
                    created_item = existing_item
                else:
                    # Crear nuevo item
                    created_item = CartItem.objects.create(cart=cart, **item_data)
                
                response_serializer = CartItemSerializer(created_item)
                return Response(response_serializer.data, status=status.HTTP_201_CREATED)
    ...
    def remove_item(self, request, session_id=None):
        ...
        try:
            ...
            item.delete()
            return Response({'message': 'Item removido exitosamente'})
    ...
    def update_item(self, request, session_id=None):
        ...
        try:
            ...
            if serializer.is_valid():
                ...
                response_serializer = CartItemSerializer(updated_item)
                return Response(response_serializer.data)
    ...
    def convert_to_order(self, request, session_id=None):
        ...
        cart = self.get_object()
        serializer = CartConvertToOrderSerializer(
            data=request.data,
            context={'cart': cart}
        )
        ...
        if serializer.is_valid():
            try:
                with transaction.atomic():
                    ...
                    if order:
                        # Retornar la orden creada con detalles completos
                ...
                return Response({
                    'error': 'Error al convertir carrito a orden',
                    'details': str(e)
                }, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    def get_or_create(self, request):
        ...
        if not session_id or not table_id:
            return Response(
                {'error': 'Se requiere session_id y table_id'}, 
                status=status.HTTP_400_BAD_REQUEST
            )
        ...
        try:
            ...
            return Response({
                'error': 'Error al obtener/crear carrito',
                'details': str(e)
            }, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class CartItemViewSet(viewsets.ModelViewSet):
    """
    API para gestión individual de items del carrito.
    Complementa CartViewSet para operaciones específicas de items.
    """
    queryset = CartItem.objects.select_related(
        'cart', 'recipe', 'container'
    ).order_by('-created_at')
