...
from .models import Order, OrderItem, OrderItemIngredient, Payment, PaymentItem, ContainerSale, Cart, CartItem
...
class CartItemSerializer(serializers.ModelSerializer):
    ...
    recipe_name = serializers.CharField(source='recipe.name', read_only=True)
    recipe_base_price = serializers.DecimalField(source='recipe.base_price', max_digits=10, decimal_places=2, read_only=True)
    recipe_preparation_time = serializers.IntegerField(source='recipe.preparation_time', read_only=True)
    container_name = serializers.CharField(source='container.name', read_only=True)
    container_price = serializers.DecimalField(source='container.price', max_digits=10, decimal_places=2, read_only=True)
    
    # Totales calculados
    food_total = serializers.SerializerMethodField()
    container_total = serializers.SerializerMethodField()
    item_total = serializers.SerializerMethodField()
    
    class Meta:
        model = CartItem
        fields = [
            'id', 'cart', 'recipe', 'recipe_name', 'recipe_base_price', 'recipe_preparation_time',
            'quantity', 'unit_price', 'notes', 'is_takeaway', 'has_taper',
            'container', 'container_name', 'container_price',
            'food_total', 'container_total', 'item_total',
            'created_at', 'updated_at'
        ]
        read_only_fields = ['id', 'created_at', 'updated_at']
    
    def get_food_total(self, obj):
        return obj.get_food_total()
    
    def get_container_total(self, obj):
        return obj.get_container_total()
...
class CartItemCreateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if data.get('has_taper', False):
            container = data.get('container')
            if not container:
                # Buscar container por defecto de la receta
                recipe = data['recipe']
                if recipe.container:
                    data['container'] = recipe.container
                else:
                    raise serializers.ValidationError("Se requiere seleccionar un envase para items para llevar")
            
            # Validar stock del container
...
class CartSerializer(serializers.ModelSerializer):
    """Serializer básico para carritos"""
    table_number = serializers.CharField(source='table.table_number', read_only=True)
    zone_name = serializers.CharField(source='table.zone.name', read_only=True)
    ...
    class Meta:
        model = Cart
        fields = [
            'id', 'session_id', 'table', 'table_number', 'zone_name', 'user',
            'total_items', 'subtotal', 'containers_total', 'grand_total',
...
class CartDetailSerializer(CartSerializer):
    """Serializer detallado para carritos con items"""
    items = CartItemSerializer(source='cartitem_set', many=True, read_only=True)
    table_detail = TableSerializer(source='table', read_only=True)
    
    class Meta(CartSerializer.Meta):
        fields = CartSerializer.Meta.fields + ['items', 'table_detail']


class CartCreateSerializer(serializers.ModelSerializer):
    """Serializer para crear carritos"""
    items = CartItemCreateSerializer(many=True, write_only=True, required=False)
    
    class Meta:
        model = Cart
        fields = ['session_id', 'table', 'user', 'items']
    
    @transaction.atomic
    def create(self, validated_data):
        items_data = validated_data.pop('items', [])
        
        # Crear o obtener carrito existente
        cart = Cart.get_or_create_for_session(
        ...
        for item_data in items_data:
            # Verificar si existe un item similar para fusionar
            existing_item = cart.cartitem_set.filter(
...
class CartConvertToOrderSerializer(serializers.Serializer):
    """Serializer para convertir carrito a orden"""
    ...
    def validate(self, data):
        ...
        # Validar stock de todos los items antes de convertir
        for cart_item in cart.cartitem_set.all():
            if not cart_item.recipe.check_availability():
                raise serializers.ValidationError(
                    f"No hay suficiente stock para {cart_item.recipe.name}"
                )
            
            # Validar stock de containers
            if cart_item.has_taper and cart_item.container:
                if cart_item.container.stock < cart_item.quantity:
                    raise serializers.ValidationError(
                        f"Stock insuficiente de {cart_item.container.name}. "
                        f"Disponible: {cart_item.container.stock}, "
                        f"Requerido: {cart_item.quantity}"
                    )
