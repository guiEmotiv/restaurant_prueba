...
class DashboardOperativoViewSet(viewsets.ViewSet):
    ...
    def _query_dashboard_view(self, selected_date):
        ...
        # PARTE 1.6: Obtener datos de containers por separado
        cursor = connection.cursor()
        try:
            cursor.execute("""
                SELECT 
                    o.id as order_id, cs.quantity, cs.unit_price, cs.total_price, c.name as container_name
                FROM "order" o
        ...
        # PARTE 2: Obtener TODOS los datos operativos del día (incluyendo PAID)
        cursor2 = connection.cursor()
        try:
            cursor2.execute("""
                SELECT 
                    o.id as order_id, o.status as order_status, o.total_amount as order_total, o.waiter,
                    oi.id as item_id, oi.status as item_status, r.name as recipe_name, g.name as category_name,
                    DATE(datetime(COALESCE(o.paid_at, o.created_at), '-5 hours')) as operational_date,
                    CASE WHEN o.status IN ('CREATED', 'SERVED') THEN 1 ELSE 0 END as is_active_order,
                    CASE WHEN oi.status = 'CREATED' THEN 1 ELSE 0 END as is_pending_item,
                    CASE WHEN oi.status = 'PREPARING' THEN 1 ELSE 0 END as is_preparing_item,
                    CASE WHEN oi.status = 'SERVED' THEN 1 ELSE 0 END as is_served_item
                FROM "order" o
                LEFT JOIN order_item oi ON o.id = oi.order_id
                LEFT JOIN recipe r ON oi.recipe_id = r.id
                LEFT JOIN "group" g ON r.group_id = g.id
                WHERE DATE(datetime(COALESCE(o.paid_at, o.created_at), '-5 hours')) = %s
                ORDER BY o.id, oi.id
            """, [selected_date])
        ...
        # Estadísticas de ingresos por tipo (comida vs envases)
        revenue_breakdown = {
            'food_revenue': Decimal('0'),      # Ingresos por comida/recetas
            'container_revenue': Decimal('0')  # Ingresos por envases
        }
        
        # PROCESAR DATOS PAID (métricas financieras)
        for row in paid_data:
            (order_id, order_total, order_status, waiter,
             item_id, quantity, unit_price, total_price, item_status, is_takeaway,
            ...
            if order_id not in paid_orders_totals:
                ...
                # Categorizar orden como delivery o restaurante (basado en si ALGÚN item es takeaway)
                # Para esto necesitamos verificar si algún item de la orden es takeaway
                pass  # Lo procesaremos después
            ...
            if item_id:  # Solo contar items válidos
                if is_takeaway:
                    delivery_stats['delivery_items'] += quantity or 0
                    
                    # Stats específicos de delivery por categoría
                    if category_name and recipe_name:
                        category = category_name or 'Sin Categoría'
                        delivery_category_stats[category]['revenue'] += Decimal(str(total_price or 0))
                        ...
                        delivery_category_stats[category]['recipes'][recipe_name]['revenue'] += Decimal(str(total_price or 0))
                else:
        ...
        # CALCULAR ESTADÍSTICAS DE DELIVERY VS RESTAURANTE POR ITEM
        # Cada item individual contribuye a local o delivery según su is_takeaway
        order_has_delivery_items = set()  # Para contar órdenes únicas con items delivery
        order_has_local_items = set()     # Para contar órdenes únicas con items local
        ...
        for row in paid_data:
            ...
            if item_id:  # Solo procesar items válidos
                if is_takeaway:
                    # Este item es delivery
                    delivery_stats['total_delivery_revenue'] += Decimal(str(total_price or 0))
                    order_has_delivery_items.add(order_id)
                else:
                    # Este item es local/restaurante
                    delivery_stats['total_restaurant_revenue'] += Decimal(str(total_price or 0))
                    order_has_local_items.add(order_id)
        
        # Contar órdenes únicas (una orden puede tener items de ambos tipos)
        ...
        # PROCESAR DATOS DE CONTAINERS por separado
        for row in container_data:
            (order_id, quantity, unit_price, total_price, container_name) = row
            
            if total_price:
                revenue_breakdown['container_revenue'] += Decimal(str(total_price))
        
        # PROCESAR DATOS OPERATIVOS (estado de items y métricas operativas)
        # Ahora operational_data incluye TODOS los items del día
        for row in operational_data:
            ...
            # Estado de items: TODOS los items del día
            if item_id and item_status:
                item_status_stats[item_status]['count'] += 1
                # Solo agregar monto si es de una orden PAID
                if order_status == 'PAID':
                    item_status_stats[item_status]['amount'] += Decimal(str(order_total or 0))
                else:
        ...
        # CALCULAR INGRESOS POR COMIDA (total - containers)
        # Los ingresos por comida son el total de la orden menos los containers
        total_food_revenue = total_revenue - revenue_breakdown['container_revenue']
        revenue_breakdown['food_revenue'] = total_food_revenue
        ...
        from inventory.models import Recipe
        sold_recipe_names = [row[10] for row in paid_data if row[10] is not None]  # recipe_name from paid_data (position 10 now)
        ...
        return {
            'summary': {
                ...
                # Revenue breakdown (food vs containers)
                'food_revenue': float(revenue_breakdown['food_revenue']),
                'container_revenue': float(revenue_breakdown['container_revenue'])
            },
