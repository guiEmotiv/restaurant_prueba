...
if [ "$ENV_TYPE" = "production" ] || [ "$ENV_TYPE" = "development_docker" ]; then
    ...
    docker exec $DOCKER_CONTAINER bash -c "cat > /app/clear_db.py << 'SCRIPT_END'
#!/usr/bin/env python3
import os
import sys
import django
...
def verify_empty_database():
    print('📊 FASE 3: Verificando base de datos vacía...')
...
def verify_auto_increment_reset():
    print('🔍 FASE 4: Verificando contadores reiniciados...')
